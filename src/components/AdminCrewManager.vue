      ...
      <table v-if="filteredCrew.length">
        ...
        <tbody>
          <tr v-for="member in filteredCrew" :key="member.id">
            ...
            <td>
              <!-- Only allow CRUD on CREW members -->
              <button
                v-if="member.role === 'CREW'"
      ...
      <div v-if="editing" class="modal">
        <div class="modal-content">
          ...
          <form @submit.prevent="saveMember">
            ...
            <input v-model="editForm.email" placeholder="Email" required type="email" />
            <input v-model="editForm.phoneNumber" placeholder="Phone Number" required pattern="\d{3}-\d{3}-\d{4}" />
            <select v-model="editForm.role" required>
              <option value="" disabled>Select Role</option>
              <option value="CREW">Crew</option>
              <!-- ADMIN option removed: cannot create/edit Admins -->
            </select>
            <input
              v-model="positionsInput"
              placeholder="Qualified Positions (comma-separated)"
              required
            />
...
// import the full DTO‐returning API methods
import {
  getAllCrewMembers,
  createCrewMember,
  updateCrewMember
...
const filteredCrew = computed(() => {
  ...
  return crew.value.filter(m =>
    ...
    (m.positions || []).some(pos => pos.toLowerCase().includes(term))
  )
})
...
async function loadCrew() {
  try {
    crew.value = await getAllCrewMembers()
  } catch (err) {
...
function editMember(member) {
  Object.assign(editForm, member)         // copy into form
  positionsInput.value = (member.positions || []).join(', ')
  isNew.value = false
  editing.value = true
}

// Prepare to add a new member
function createNewMember() {
  Object.assign(editForm, {
    id: null, firstName: '', lastName: '',
    email: '', phoneNumber: '', role: '', positions: []
  })
...
async function saveMember() {
  try {
    const payload = {
      ...
      role: editForm.role,
      positions: positionsInput.value.split(',').map(s => s.trim())
    }
...
async function deleteMember(member) {
  ...
  // Ideally call a DELETE endpoint; for now just remove from list:
